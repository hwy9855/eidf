...
from transformers import BartForConditionalGeneration, RagRetriever, RagTokenForGeneration
from transformers import logging as transformers_logging

from model import RagAEForGeneration
...
def metric_max_over_ground_truths_rouge(metric_fn, prediction, ground_truths):
    return max(ROUGE(prediction, gt)[metric_fn] for gt in ground_truths)
...
def get_scores(args, preds_path, gold_data_path):
    ...
    f1 = em = bleu1 = bleu4 = rouge1 = rouge2 = rougeL = total = 0
    for prediction, ground_truths in zip(hypos, answers):
        total += 1
        em += metric_max_over_ground_truths(exact_match_score, prediction, ground_truths)
        bleu1 += metric_max_over_ground_truths(BLEU1, prediction, [ground_truths])
        bleu4 += metric_max_over_ground_truths(BLEU4, prediction, [ground_truths])
        rouge1 += metric_max_over_ground_truths_rouge('rouge1_recall', prediction, ground_truths)
        rouge2 += metric_max_over_ground_truths_rouge('rouge2_recall', prediction, ground_truths)
        rougeL += metric_max_over_ground_truths_rouge('rougeL_recall', prediction, ground_truths)

    em = 100.0 * em / total
    f1 = 100.0 * f1 / total
    bleu1 = 100.0 * bleu1 / total
    bleu4 = 100.0 * bleu4 / total
    rouge1 = 100.0 * rouge1 / total
    rouge2 = 100.0 * rouge2 / total
    rougeL = 100.0 * rougeL / total

    logger.info(f"F1: {f1:.2f}")
    logger.info(f"EM: {em:.2f}")
    logger.info(f"BLEU-1: {bleu1:.2f}")
    logger.info(f"BLEU-4: {bleu4:.2f}")
    logger.info(f"ROUGE-1: {rouge1:.2f}")
    logger.info(f"ROUGE-2: {rouge2:.2f}")
    logger.info(f"ROUGE-L: {rougeL:.2f}")
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Path to the retrieval passage",
    )
    ...
    parser.add_argument("--min_length", default=5, type=int, help="Min length of the generated answers")
...
def main(args):
    ...
    if args.model_type.startswith("rag"):
        model_class = RagTokenForGeneration if args.model_type == "rag" else RagAEForGeneration
