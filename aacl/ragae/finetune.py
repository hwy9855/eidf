...
class GenerativeQAModule(BaseTransformer):
    mode = "generative_qa"
    loss_names = ["loss"]
    metric_names = ["em", "bleu-1", "bleu-4", "rouge-1", "rouge-2", "rouge-L"]
    # metric_names = ["em"]
    ...
    def __init__(self, hparams, **kwargs):
        ...
        if hparams.model_type == "rag":
            self.model_class = RagTokenForGeneration
        elif hparams.model_type == "bart":
            self.model_class = BartForConditionalGeneration
        elif hparams.model_type == "ragae":
            self.model_class = RagAEForGeneration
        else:
            self.model_class = T5ForConditionalGeneration
        self.is_rag_model = is_rag_model(hparams.model_type)

        self.val_metric = hparams.val_metric
        ...
        config.ragae_type = hparams.ragae_type
        config.with_nll = hparams.with_nll
        config.freeze_rag = hparams.freeze_rag
        config.n_docs = hparams.n_docs
        config.bsz = hparams.train_batch_size
    ...
    def _step(self, batch: dict) -> Tuple:
        source_ids, source_mask, target_ids = \
            batch["input_ids"], batch["attention_mask"], batch["decoder_input_ids"]
        rag_kwargs = {}
    ...
    def validation_epoch_end(self, outputs, prefix="val") -> Dict:
        ...
        self.log(f"{prefix}_{self.val_metric}", metrics_tensor)
        self.log('step_count', self.step_count)
    ...
    def calc_generative_metrics(self, preds, target) -> Dict:
        rouge_res = ROUGE(preds, target)
        return {"em": calculate_exact_match(preds, target)["em"],
                "bleu-4": float(BLEU4(preds, [target])),
                "bleu-1": float(BLEU1(preds, [target])),
                "rouge-1": float(rouge_res['rouge1_recall']),
                "rouge-2": float(rouge_res['rouge2_recall']),
                "rouge-L": float(rouge_res['rougeL_recall']),
                }
    ...
    def get_dataset(self, type_path) -> Seq2SeqDatasetAAP:
        n_obs = self.n_obs[type_path]
        max_target_length = self.target_lens[type_path]
        dataset = Seq2SeqDatasetAAP(
            self.tokenizer,
            type_path=type_path,
            n_obs=n_obs,
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Whether to train the retriever with the marginalized output"
            )
        )
        parser.add_argument(
            "--freeze_rag",
            action="store_true",
            help=(
                "Whether to freeze the training of RAG"
            )
        )
        parser.add_argument(
            "--sba_path",
            type=str,
            default='../sba/results/checkpoint-100000/pytorch_model.bin',
            help=(
                "Identify the path for pretrained sba"
            )
        )
        parser.add_argument(
            "--val_metric",
            choices=["em", "bleu-1", "bleu-4", "rouge-1", "rouge-2", "rouge-L", "loss"],
            type=str,
            default="rouge-L"
        )
        parser.add_argument(
